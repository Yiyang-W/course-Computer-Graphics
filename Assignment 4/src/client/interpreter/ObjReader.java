...
//	boolean isInCameraWorld;
	...
	private void transformNormals(){
		for (Point3DH vertex :
				objNormals) {
			Point3DH transformedVertex = SimpInterpreter.objTransformNormal(simpInterpreter, vertex);
			transformedVertex = normalizeNormal(transformedVertex);
			transformedNormals.add(transformedVertex);
		}
	}
...
//			List<Vertex3D> ls = new ArrayList<>();
			...
			Point3DH normal;
//			Point3DH noNormal = new Point3DH(0, 0, 0);
			vertices[0] = transformedVertices.get(face.get(0).vertexIndex-1);
			if (face.get(0).normalIndex > 0) {
				normal = transformedNormals.get(face.get(0).normalIndex-1);
//				System.out.println(normal);
			...
			for (int i = 1; i < face.size()-1; ++i) {
				...
				if (face.get(i).normalIndex > 0) {
					normal = transformedNormals.get(face.get(i).normalIndex-1);
					vertices[1].setNormal(normal);
//					System.out.println(normal);

				}
				if (face.get(i+1).normalIndex > 0) {
					normal = transformedNormals.get(face.get(i+1).normalIndex-1);
//					System.out.println(normal);
